...
function ChatListRow({chatId} : {chatId: string}) {
  ...
  <div className='flex-1'>
    <p className='font-bold'>
      {!message && "New Chat"}
      {message && [message?.user.name || session?.user.name].toString().toString().split(" ")[0]}
    </p>

    <p className='text-gray-400 line-clamp-1'>
      Get the converstaion started...
    </p>
  </div>

  <div className='text-xs text-gray-400 text-right'>
    <p className='mb-auto'>
      {message ? new Date(message.timestamp).toLocaleDateString() :
      "No messages yet"}
    </p>
